'''
date        : 31/03/2020
description : this module creates a config and extracts a dataset from
              an existing model.
...
if os.path.isfile("{base}/config.py".format(base = sys.argv[1])):
    ...
    if config.Keep_Log:
        ...
        log.info("config has been found", keep_log)
    else:
        keep_log = False

    specified_name = config.Project_Name in list_folders(sys.argv[1])
    
    if config.Model_2_config == False:
        log.info("Model_2_config is set to 'False'", keep_log)
        sys.exit(0)

    if not os.path.isdir("{base}/data/old_configs".format(base = sys.argv[1])):
        os.makedirs("{base}/data/old_configs".format(base = sys.argv[1]))

    count = 1
    while True:
        if not os.path.isfile("{base}/data/old_configs/config_bu_{count}.py".format(
                base = sys.argv[1], count = count
            )):
            log.info("Backing up old config to {base}/data/old_configs/config_bu" \
                "_{count}.py".format(base = sys.argv[1], count = count), keep_log)

            shutil.copyfile("{base}/config.py".format(base = sys.argv[1]),
                "{base}/data/old_configs/config_bu_{count}.py".format(
                    base = sys.argv[1], count = count
                )
            )
            break
...
if not specified_name:
    ...
    while True:
        print("\t> for which model do you want to get config ('E' then 'Enter' to exit)")
        count = 0
    ...
    selected_model = config.Project_Name
    log.info("specified source model from config: {m}".format(m = selected_model), keep_log)

# announce
print("\n     >> generating config and extracting data")
...
# write config
log.info("writing config file", keep_log)
write_to(
    "{base}/config.py".format(base = sys.argv[1]),
...
print("\t-> finnished generating config...\n")
sys.exit(1)
