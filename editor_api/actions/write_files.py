...
class WriteFiles(ExecutableApi):
	...
	def write_simulation(self, start_prog, allocated_prog):
		...
		if time_sim_file != NULL_FILE:
			...
			simulation.Time_sim(os.path.join(self.__dir, time_sim_file), self.__version, self.__swat_version).write()

		prog += prog_step
		print_prt_file = files[1].strip()
		if print_prt_file != NULL_FILE:
			self.update_file_status(start_prog, print_prt_file)
			simulation.Print_prt(os.path.join(self.__dir, print_prt_file), self.__version, self.__swat_version).write()

		prog += prog_step
		object_prt_file = files[2].strip()
		if object_prt_file != NULL_FILE:
			self.update_file_status(start_prog, object_prt_file)
			simulation.Object_prt(os.path.join(self.__dir, object_prt_file), self.__version, self.__swat_version).write()

		prog += prog_step
		object_cnt_file = files[3].strip()
		if object_cnt_file != NULL_FILE:
			self.update_file_status(start_prog, object_cnt_file)
			simulation.Object_cnt(os.path.join(self.__dir, object_cnt_file), self.__version, self.__swat_version).write()

		prog += prog_step
		constituents_cs_file = files[4].strip()
		if constituents_cs_file != NULL_FILE:
			self.update_file_status(start_prog, constituents_cs_file)
			simulation.Constituents_cs(os.path.join(self.__dir, constituents_cs_file), self.__version, self.__swat_version).write()
	...
	def write_climate(self, start_prog, allocated_prog):
		...
		if weather_wgn_file != NULL_FILE:
			...
			climate.Weather_wgn_cli(os.path.join(self.__dir, weather_wgn_file), self.__version, self.__swat_version).write()
	...
	def write_connect(self, start_prog, allocated_prog):
		...
		if hru_con_file != NULL_FILE:
			...
			connect.Hru_con(os.path.join(self.__dir, hru_con_file), self.__version, self.__swat_version).write()

		prog += prog_step
		hru_lte_con_file = files[1].strip()
		if hru_lte_con_file != NULL_FILE:
			self.update_file_status(prog, hru_lte_con_file)
			connect.Hru_lte_con(os.path.join(self.__dir, hru_lte_con_file), self.__version, self.__swat_version).write()

		prog += prog_step
		rout_unit_con_file = files[2].strip()
		if rout_unit_con_file != NULL_FILE:
			self.update_file_status(prog, rout_unit_con_file)
			connect.Rout_unit_con(os.path.join(self.__dir, rout_unit_con_file), self.__version, self.__swat_version).write()

		prog += prog_step
		aquifer_con_file = files[4].strip()
		if aquifer_con_file != NULL_FILE:
			self.update_file_status(prog, aquifer_con_file)
			connect.Aquifer_con(os.path.join(self.__dir, aquifer_con_file), self.__version, self.__swat_version).write()

		prog += prog_step
		channel_con_file = files[6].strip()
		if channel_con_file != NULL_FILE:
			self.update_file_status(prog, channel_con_file)
			connect.Channel_con(os.path.join(self.__dir, channel_con_file), self.__version, self.__swat_version).write()

		prog += prog_step
		reservoir_con_file = files[7].strip()
		if reservoir_con_file != NULL_FILE:
			self.update_file_status(prog, reservoir_con_file)
			connect.Reservoir_con(os.path.join(self.__dir, reservoir_con_file), self.__version, self.__swat_version).write()

		prog += prog_step
		recall_con_file = files[8].strip()
		if recall_con_file != NULL_FILE:
			self.update_file_status(prog, recall_con_file)
			connect.Recall_con(os.path.join(self.__dir, recall_con_file), self.__version, self.__swat_version).write()

		prog += prog_step
		exco_con_file = files[9].strip()
		if exco_con_file != NULL_FILE:
			self.update_file_status(prog, exco_con_file)
			connect.Exco_con(os.path.join(self.__dir, exco_con_file), self.__version, self.__swat_version).write()
		...
		if chandeg_con_file != NULL_FILE:
			...
			connect.Chandeg_con(os.path.join(self.__dir, chandeg_con_file), self.__version, self.__swat_version).write()
	...
	def write_channel(self, start_prog, allocated_prog):
		...
		if channel_cha_file != NULL_FILE:
			...
			channel.Channel_cha(os.path.join(self.__dir, channel_cha_file), self.__version, self.__swat_version).write()

		prog += prog_step
		initial_cha_file = files[0].strip()
		if initial_cha_file != NULL_FILE:
			self.update_file_status(prog, initial_cha_file)
			channel.Initial_cha(os.path.join(self.__dir, initial_cha_file), self.__version, self.__swat_version).write()

		prog += prog_step
		hydrology_cha_file = files[2].strip()
		if hydrology_cha_file != NULL_FILE:
			self.update_file_status(prog, hydrology_cha_file)
			channel.Hydrology_cha(os.path.join(self.__dir, hydrology_cha_file), self.__version, self.__swat_version).write()

		prog += prog_step
		sediment_cha_file = files[3].strip()
		if sediment_cha_file != NULL_FILE:
			self.update_file_status(prog, sediment_cha_file)
			channel.Sediment_cha(os.path.join(self.__dir, sediment_cha_file), self.__version, self.__swat_version).write()

		prog += prog_step
		nutrients_cha_file = files[4].strip()
		if nutrients_cha_file != NULL_FILE:
			self.update_file_status(prog, nutrients_cha_file)
			channel.Nutrients_cha(os.path.join(self.__dir, nutrients_cha_file), self.__version, self.__swat_version).write()

		prog += prog_step
		channel_lte_cha_file = files[5].strip()
		if channel_lte_cha_file != NULL_FILE:
			self.update_file_status(prog, channel_lte_cha_file)
			channel.Channel_lte_cha(os.path.join(self.__dir, channel_lte_cha_file), self.__version, self.__swat_version).write()

		prog += prog_step
		hyd_sed_lte_cha_file = files[6].strip()
		if hyd_sed_lte_cha_file != NULL_FILE:
			self.update_file_status(prog, hyd_sed_lte_cha_file)
			channel.Hyd_sed_lte_cha(os.path.join(self.__dir, hyd_sed_lte_cha_file), self.__version, self.__swat_version).write()
	...
	def write_reservoir(self, start_prog, allocated_prog):
		...
		if initial_res_file != NULL_FILE:
			...
			reservoir.Initial_res(os.path.join(self.__dir, initial_res_file), self.__version, self.__swat_version).write()

		prog += prog_step
		reservoir_res_file = files[1].strip()
		if reservoir_res_file != NULL_FILE:
			self.update_file_status(prog, reservoir_res_file)
			reservoir.Reservoir_res(os.path.join(self.__dir, reservoir_res_file), self.__version, self.__swat_version).write()

		prog += prog_step
		hydrology_res_file = files[2].strip()
		if hydrology_res_file != NULL_FILE:
			self.update_file_status(prog, hydrology_res_file)
			reservoir.Hydrology_res(os.path.join(self.__dir, hydrology_res_file), self.__version, self.__swat_version).write()

		prog += prog_step
		sediment_res_file = files[3].strip()
		if sediment_res_file != NULL_FILE:
			self.update_file_status(prog, sediment_res_file)
			reservoir.Sediment_res(os.path.join(self.__dir, sediment_res_file), self.__version, self.__swat_version).write()

		prog += prog_step
		nutrients_res_file = files[4].strip()
		if nutrients_res_file != NULL_FILE:
			self.update_file_status(prog, nutrients_res_file)
			reservoir.Nutrients_res(os.path.join(self.__dir, nutrients_res_file), self.__version, self.__swat_version).write()

		prog += prog_step
		weir_res_file = files[5].strip()
		if weir_res_file != NULL_FILE:
			self.update_file_status(prog, weir_res_file)
			reservoir.Weir_res(os.path.join(self.__dir, weir_res_file), self.__version, self.__swat_version).write()

		prog += prog_step
		wetland_wet_file = files[6].strip()
		if wetland_wet_file != NULL_FILE:
			self.update_file_status(prog, wetland_wet_file)
			reservoir.Wetland_wet(os.path.join(self.__dir, wetland_wet_file), self.__version, self.__swat_version).write()

		prog += prog_step
		hydrology_wet_file = files[7].strip()
		if hydrology_wet_file != NULL_FILE:
			self.update_file_status(prog, hydrology_wet_file)
			reservoir.Hydrology_wet(os.path.join(self.__dir, hydrology_wet_file), self.__version, self.__swat_version).write()
	...
	def write_routing_unit(self, start_prog, allocated_prog):
		...
		if rout_unit_def_file != NULL_FILE:
			...
			routing_unit.Rout_unit_def(os.path.join(self.__dir, rout_unit_def_file), self.__version, self.__swat_version).write()

		prog += prog_step
		rout_unit_ele_file = files[1].strip()
		if rout_unit_ele_file != NULL_FILE:
			self.update_file_status(prog, rout_unit_ele_file)
			routing_unit.Rout_unit_ele(os.path.join(self.__dir, rout_unit_ele_file), self.__version, self.__swat_version).write()

		rout_unit_ru_file = files[2].strip()
		if rout_unit_ru_file != NULL_FILE:
			self.update_file_status(prog, rout_unit_ru_file)
			routing_unit.Rout_unit(os.path.join(self.__dir, rout_unit_ru_file), self.__version, self.__swat_version).write()

		prog += prog_step
		rout_unit_dr_file = files[3].strip()
		if rout_unit_dr_file != NULL_FILE:
			self.update_file_status(prog, rout_unit_dr_file)
			routing_unit.Rout_unit_dr(os.path.join(self.__dir, rout_unit_dr_file), self.__version, self.__swat_version).write()
	...
	def write_hru(self, start_prog, allocated_prog):
		...
		if hru_data_file != NULL_FILE:
			...
			hru.Hru_data_hru(os.path.join(self.__dir, hru_data_file), self.__version, self.__swat_version).write()

		prog += prog_step
		hru_lte_hru_file = files[1].strip()
		if hru_lte_hru_file != NULL_FILE:
			self.update_file_status(prog, hru_lte_hru_file)
			hru.Hru_lte_hru(os.path.join(self.__dir, hru_lte_hru_file), self.__version, self.__swat_version).write()
	...
	def write_dr(self, start_prog, allocated_prog):
		...
		if delratio_del_file != NULL_FILE:
			...
			dr.Delratio_del(os.path.join(self.__dir, delratio_del_file), self.__version, self.__swat_version).write()

		prog += prog_step
		dr_om_file = files[1].strip()
		if dr_om_file != NULL_FILE:
			self.update_file_status(prog, dr_om_file)
			dr.Dr_om_del(os.path.join(self.__dir, dr_om_file), self.__version, self.__swat_version).write()
	...
	def write_aquifer(self, start_prog, allocated_prog):
		...
		if aquifer_aqu_file != NULL_FILE:
			...
			aquifer.Aquifer_aqu(os.path.join(self.__dir, aquifer_aqu_file), self.__version, self.__swat_version).write()

		prog += prog_step
		initial_aqu_file = files[0].strip()
		if initial_aqu_file != NULL_FILE:
			self.update_file_status(prog, initial_aqu_file)
			aquifer.Initial_aqu(os.path.join(self.__dir, initial_aqu_file), self.__version, self.__swat_version).write()
	...
	def write_basin(self, start_prog, allocated_prog):
		...
		if codes_bsn_file != NULL_FILE:
			...
			basin.Codes_bsn(os.path.join(self.__dir, codes_bsn_file), self.__version, self.__swat_version).write()
	...
	def write_hydrology(self, start_prog, allocated_prog):
		...
		if hydrology_hyd_file != NULL_FILE:
			...
			hydrology.Hydrology_hyd(os.path.join(self.__dir, hydrology_hyd_file), self.__version, self.__swat_version).write()
		...
		if topography_hyd_file != NULL_FILE:
			...
			hydrology.Topography_hyd(os.path.join(self.__dir, topography_hyd_file), self.__version, self.__swat_version).write()
	...
	def write_exco(self, start_prog, allocated_prog):
		...
		if exco_exc_file != NULL_FILE:
			...
			exco.Exco_exc(os.path.join(self.__dir, exco_exc_file), self.__version, self.__swat_version).write()
		...
		if exco_pest_file != NULL_FILE:
			...
			exco.Exco_pest_exc(os.path.join(self.__dir, exco_pest_file), self.__version, self.__swat_version).write()

		prog += prog_step
		exco_path_file = files[3].strip()
		if exco_path_file != NULL_FILE:
			self.update_file_status(prog, exco_path_file)
			exco.Exco_path_exc(os.path.join(self.__dir, exco_path_file), self.__version, self.__swat_version).write()

		prog += prog_step
		exco_hmet_file = files[4].strip()
		if exco_hmet_file != NULL_FILE:
			self.update_file_status(prog, exco_hmet_file)
			exco.Exco_hmet_exc(os.path.join(self.__dir, exco_hmet_file), self.__version, self.__swat_version).write()

		prog += prog_step
		exco_salt_file = files[5].strip()
		if exco_salt_file != NULL_FILE:
			self.update_file_status(prog, exco_salt_file)
			exco.Exco_salt_exc(os.path.join(self.__dir, exco_salt_file), self.__version, self.__swat_version).write()
	...
	def write_recall(self, start_prog, allocated_prog):
		...
		if recall_rec_file != NULL_FILE:
			...
			recall.Recall_rec(os.path.join(self.__dir, recall_rec_file), self.__version, self.__swat_version).write()
	...
	def write_structural(self, start_prog, allocated_prog):
		...
		if tiledrain_str_file != NULL_FILE:
			...
			structural.Tiledrain_str(os.path.join(self.__dir, tiledrain_str_file), self.__version, self.__swat_version).write()

		prog += prog_step
		septic_str_file = files[1].strip()
		if septic_str_file != NULL_FILE:
			self.update_file_status(prog, septic_str_file)
			structural.Septic_str(os.path.join(self.__dir, septic_str_file), self.__version, self.__swat_version).write()

		prog += prog_step
		filterstrip_str_file = files[2].strip()
		if filterstrip_str_file != NULL_FILE:
			self.update_file_status(prog, filterstrip_str_file)
			structural.Filterstrip_str(os.path.join(self.__dir, filterstrip_str_file), self.__version, self.__swat_version).write()

		prog += prog_step
		grassedww_str_file = files[3].strip()
		if grassedww_str_file != NULL_FILE:
			self.update_file_status(prog, grassedww_str_file)
			structural.Grassedww_str(os.path.join(self.__dir, grassedww_str_file), self.__version, self.__swat_version).write()

		prog += prog_step
		bmpuser_str_file = files[4].strip()
		if bmpuser_str_file != NULL_FILE:
			self.update_file_status(prog, bmpuser_str_file)
			structural.Bmpuser_str(os.path.join(self.__dir, bmpuser_str_file), self.__version, self.__swat_version).write()
	...
	def write_parm_db(self, start_prog, allocated_prog):
		...
		if plants_plt_file != NULL_FILE:
			...
			hru_parm_db.Plants_plt(os.path.join(self.__dir, plants_plt_file), self.__version, self.__swat_version).write()

		prog += prog_step
		fertilizer_frt_file = files[1].strip()
		if fertilizer_frt_file != NULL_FILE:
			self.update_file_status(prog, fertilizer_frt_file)
			hru_parm_db.Fertilizer_frt(os.path.join(self.__dir, fertilizer_frt_file), self.__version, self.__swat_version).write()

		prog += prog_step
		tillage_til_file = files[2].strip()
		if tillage_til_file != NULL_FILE:
			self.update_file_status(prog, tillage_til_file)
			hru_parm_db.Tillage_til(os.path.join(self.__dir, tillage_til_file), self.__version, self.__swat_version).write()

		prog += prog_step
		pesticide_pst_file = files[3].strip()
		if pesticide_pst_file != NULL_FILE:
			self.update_file_status(prog, pesticide_pst_file)
			hru_parm_db.Pesticide_pst(os.path.join(self.__dir, pesticide_pst_file), self.__version, self.__swat_version).write()

		prog += prog_step
		pathogens_pth_file = files[4].strip()
		if pathogens_pth_file != NULL_FILE:
			self.update_file_status(prog, pathogens_pth_file)
			hru_parm_db.Pathogens_pth(os.path.join(self.__dir, pathogens_pth_file), self.__version, self.__swat_version).write()

		prog += prog_step
		urban_urb_file = files[7].strip()
		if urban_urb_file != NULL_FILE:
			self.update_file_status(prog, urban_urb_file)
			hru_parm_db.Urban_urb(os.path.join(self.__dir, urban_urb_file), self.__version, self.__swat_version).write()

		prog += prog_step
		septic_sep_file = files[8].strip()
		if septic_sep_file != NULL_FILE:
			self.update_file_status(prog, septic_sep_file)
			hru_parm_db.Septic_sep(os.path.join(self.__dir, septic_sep_file), self.__version, self.__swat_version).write()

		prog += prog_step
		snow_sno_file = files[9].strip()
		if snow_sno_file != NULL_FILE:
			self.update_file_status(prog, snow_sno_file)
			hru_parm_db.Snow_sno(os.path.join(self.__dir, snow_sno_file), self.__version, self.__swat_version).write()
	...
	def write_ops(self, start_prog, allocated_prog):
		...
		if harv_ops_file != NULL_FILE:
			...
			ops.Harv_ops(os.path.join(self.__dir, harv_ops_file), self.__version, self.__swat_version).write()

		prog += prog_step
		graze_ops_file = files[1].strip()
		if graze_ops_file != NULL_FILE:
			self.update_file_status(prog, graze_ops_file)
			ops.Graze_ops(os.path.join(self.__dir, graze_ops_file), self.__version, self.__swat_version).write()

		prog += prog_step
		irr_ops_file = files[2].strip()
		if irr_ops_file != NULL_FILE:
			self.update_file_status(prog, irr_ops_file)
			ops.Irr_ops(os.path.join(self.__dir, irr_ops_file), self.__version, self.__swat_version).write()

		prog += prog_step
		chem_app_ops_file = files[3].strip()
		if chem_app_ops_file != NULL_FILE:
			self.update_file_status(prog, chem_app_ops_file)
			ops.Chem_app_ops(os.path.join(self.__dir, chem_app_ops_file), self.__version, self.__swat_version).write()

		prog += prog_step
		fire_ops_file = files[4].strip()
		if fire_ops_file != NULL_FILE:
			self.update_file_status(prog, fire_ops_file)
			ops.Fire_ops(os.path.join(self.__dir, fire_ops_file), self.__version, self.__swat_version).write()

		prog += prog_step
		sweep_ops_file = files[5].strip()
		if sweep_ops_file != NULL_FILE:
			self.update_file_status(prog, sweep_ops_file)
			ops.Sweep_ops(os.path.join(self.__dir, sweep_ops_file), self.__version, self.__swat_version).write()
	...
	def write_lum(self, start_prog, allocated_prog):
		...
		if landuse_lum_file != NULL_FILE:
			...
			lum.Landuse_lum(os.path.join(self.__dir, landuse_lum_file), self.__version, self.__swat_version).write()

		prog += prog_step
		management_sch_file = files[1].strip()
		if management_sch_file != NULL_FILE:
			self.update_file_status(prog, management_sch_file)
			lum.Management_sch(os.path.join(self.__dir, management_sch_file), self.__version, self.__swat_version).write()

		prog += prog_step
		cntable_lum_file = files[2].strip()
		if cntable_lum_file != NULL_FILE:
			self.update_file_status(prog, cntable_lum_file)
			lum.Cntable_lum(os.path.join(self.__dir, cntable_lum_file), self.__version, self.__swat_version).write()

		prog += prog_step
		cons_prac_lum_file = files[3].strip()
		if cons_prac_lum_file != NULL_FILE:
			self.update_file_status(prog, cons_prac_lum_file)
			lum.Cons_prac_lum(os.path.join(self.__dir, cons_prac_lum_file), self.__version, self.__swat_version).write()

		prog += prog_step
		ovn_table_lum_file = files[4].strip()
		if ovn_table_lum_file != NULL_FILE:
			self.update_file_status(prog, ovn_table_lum_file)
			lum.Ovn_table_lum(os.path.join(self.__dir, ovn_table_lum_file), self.__version, self.__swat_version).write()
	...
	def write_chg(self, start_prog, allocated_prog):
		...
		if cal_parms_cal_file != NULL_FILE:
			...
			change.Cal_parms_cal(os.path.join(self.__dir, cal_parms_cal_file), self.__version, self.__swat_version).write()

		prog += prog_step
		calibration_cal_file = files[1].strip()
		if calibration_cal_file != NULL_FILE:
			self.update_file_status(prog, calibration_cal_file)
			change.Calibration_cal(os.path.join(self.__dir, calibration_cal_file), self.__version, self.__swat_version).write()

		prog += prog_step
		codes_sft_file = files[2].strip()
		if codes_sft_file != NULL_FILE:
			self.update_file_status(prog, codes_sft_file)
			change.Codes_sft(os.path.join(self.__dir, codes_sft_file), self.__version, self.__swat_version).write()

		prog += prog_step
		wb_parms_sft_file = files[3].strip()
		if wb_parms_sft_file != NULL_FILE:
			self.update_file_status(prog, wb_parms_sft_file)
			change.Wb_parms_sft(os.path.join(self.__dir, wb_parms_sft_file), self.__version, self.__swat_version).write()
		...
		if water_balance_sft_file != NULL_FILE:
			...
			change.Water_balance_sft(os.path.join(self.__dir, water_balance_sft_file), self.__version, self.__swat_version).write()
		...
		if ch_sed_budget_sft_file != NULL_FILE:
			...
			change.Ch_sed_budget_sft(os.path.join(self.__dir, ch_sed_budget_sft_file), self.__version, self.__swat_version).write()

		prog += prog_step
		chsed_parms_sft_file = files[6].strip()
		if chsed_parms_sft_file != NULL_FILE:
			self.update_file_status(prog, chsed_parms_sft_file)
			change.Ch_sed_parms_sft(os.path.join(self.__dir, chsed_parms_sft_file), self.__version, self.__swat_version).write()

		prog += prog_step
		plant_parms_sft_file = files[7].strip()
		if plant_parms_sft_file != NULL_FILE:
			self.update_file_status(prog, plant_parms_sft_file)
			change.Plant_parms_sft(os.path.join(self.__dir, plant_parms_sft_file), self.__version, self.__swat_version).write()

		prog += prog_step
		plant_gro_sft_file = files[8].strip()
		if plant_gro_sft_file != NULL_FILE:
			self.update_file_status(prog, plant_gro_sft_file)
			change.Plant_gro_sft(os.path.join(self.__dir, plant_gro_sft_file), self.__version, self.__swat_version).write()
	...
	def write_init(self, start_prog, allocated_prog):
		...
		if initial_plt_file != NULL_FILE:
			...
			init.Plant_ini(os.path.join(self.__dir, initial_plt_file), self.__version, self.__swat_version).write()

		prog += prog_step
		soil_plant_ini_file = files[1].strip()
		if soil_plant_ini_file != NULL_FILE:
			self.update_file_status(prog, soil_plant_ini_file)
			init.Soil_plant_ini(os.path.join(self.__dir, soil_plant_ini_file), self.__version, self.__swat_version).write()

		prog += prog_step
		om_water_ini_file = files[2].strip()
		if om_water_ini_file != NULL_FILE:
			self.update_file_status(prog, om_water_ini_file)
			init.Om_water_ini(os.path.join(self.__dir, om_water_ini_file), self.__version, self.__swat_version).write()

		prog += prog_step
		pest_hru_ini_file = files[3].strip()
		if pest_hru_ini_file != NULL_FILE:
			self.update_file_status(prog, pest_hru_ini_file)
			init.Pest_hru_ini(os.path.join(self.__dir, pest_hru_ini_file), self.__version, self.__swat_version).write()

		prog += prog_step
		pest_water_ini_file = files[4].strip()
		if pest_water_ini_file != NULL_FILE:
			self.update_file_status(prog, pest_water_ini_file)
			init.Pest_water_ini(os.path.join(self.__dir, pest_water_ini_file), self.__version, self.__swat_version).write()

		prog += prog_step
		path_hru_ini_file = files[5].strip()
		if path_hru_ini_file != NULL_FILE:
			self.update_file_status(prog, path_hru_ini_file)
			init.Path_hru_ini(os.path.join(self.__dir, path_hru_ini_file), self.__version, self.__swat_version).write()
	...
	def write_soils(self, start_prog, allocated_prog):
		...
		if nutrients_sol_file != NULL_FILE:
			...
			soils.Nutrients_sol(os.path.join(self.__dir, nutrients_sol_file), self.__version, self.__swat_version).write()

		prog += prog_step
		soils_sol_file = files[0].strip()
		if soils_sol_file != NULL_FILE:
			self.update_file_status(prog, soils_sol_file)
			soils.Soils_sol(os.path.join(self.__dir, soils_sol_file), self.__version, self.__swat_version).write()
		...
		if self.__is_lte:
			...
			if soils_lte_sol_file != NULL_FILE:
				...
				soils.Soils_lte_sol(os.path.join(self.__dir, soils_lte_sol_file), self.__version, self.__swat_version).write()
	...
	def write_decision_table(self, start_prog, allocated_prog):
		...
		if lum_dtl_file != NULL_FILE:
			...
			decision_table.D_table_dtl(os.path.join(self.__dir, lum_dtl_file), self.__version, self.__swat_version).write()

		prog += prog_step
		res_rel_dtl_file = files[1].strip()
		if res_rel_dtl_file != NULL_FILE:
			self.update_file_status(prog, res_rel_dtl_file)
			decision_table.D_table_dtl(os.path.join(self.__dir, res_rel_dtl_file), self.__version, self.__swat_version).write()

		prog += prog_step
		scen_lu_dtl_file = files[2].strip()
		if scen_lu_dtl_file != NULL_FILE:
			self.update_file_status(prog, scen_lu_dtl_file)
			decision_table.D_table_dtl(os.path.join(self.__dir, scen_lu_dtl_file), self.__version, self.__swat_version).write()

		prog += prog_step
		flo_con_dtl_file = files[3].strip()
		if flo_con_dtl_file != NULL_FILE:
			self.update_file_status(prog, flo_con_dtl_file)
			decision_table.D_table_dtl(os.path.join(self.__dir, flo_con_dtl_file), self.__version, self.__swat_version).write()
	...
	def write_regions(self, start_prog, allocated_prog):
		...
		if ls_unit_ele_file != NULL_FILE:
			...
			regions.Ls_unit_ele(os.path.join(self.__dir, ls_unit_ele_file), self.__version, self.__swat_version).write()

		prog += prog_step
		ls_unit_def_file = files[1].strip()
		if ls_unit_def_file != NULL_FILE:
			self.update_file_status(start_prog, ls_unit_def_file)
			regions.Ls_unit_def(os.path.join(self.__dir, ls_unit_def_file), self.__version, self.__swat_version).write()
