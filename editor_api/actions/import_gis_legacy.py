...
class GisImport(ExecutableApi):
	...
	def update_gis_routing(self):
		conn = db_lib.open_db(self.project_db_file)
		cols = db_lib.get_column_names(conn, 'gis_routing')
		col_names = [v['name'] for v in cols]
		if 'hyd_typ' not in col_names:
			migrator = SqliteMigrator(SqliteDatabase(self.project_db_file))
			migrate(
				migrator.add_column('gis_routing', 'hyd_typ', CharField(null=True))
			)
	...
	def get_outlet_sub_map(self):
		outlets = gis.Gis_routing.select(gis.Gis_routing.sourceid, gis.Gis_routing.sourcecat, gis.Gis_routing.sinkid, gis.Gis_routing.sinkcat, gis.Gis_routing.percent).where(gis.Gis_routing.sinkcat == RouteCat.OUTLET)
		outlet_sub_ids = []
		outlet_sub_map = {}
		...
		if exco.Exco_exc.select().count() == 0:
			...
			if exco_query.count() > 0:
				...
				for row in exco_query:
					...
					used = dict()
					for con in gis.Gis_routing.select(gis.Gis_routing.sourceid, gis.Gis_routing.sourcecat, gis.Gis_routing.sinkid, gis.Gis_routing.sinkcat, gis.Gis_routing.percent).where(
	...
	def get_con_out(self, sinkid, sinkcat, percent, order, attach_key, attach_id, is_lte=False, override_frac=False, is_lte_cha_type=False):
		...
		if sinkcat == RouteCat.PT:
			con_outs = []
			for row in gis.Gis_routing.select(gis.Gis_routing.sourceid, gis.Gis_routing.sourcecat, gis.Gis_routing.sinkid, gis.Gis_routing.sinkcat, gis.Gis_routing.percent).where(
