...
from fileio import recall, decision_table, lum
...
class ImportExportData(ExecutableApi):
	def __init__(self, file_name, table_name, db_file, delete_existing=False, related_id=0, ignore_id_col=True):
		self.__abort = False
	...
	def import_csv(self):
		if self.table_name == 'rec_dat':
			recall.Recall_rec(self.file_name).read_data(self.related_id, self.delete_existing)
	...
	def export_csv(self):
		if self.table_name in dtl_names:
			decision_table.D_table_dtl(self.file_name, file_type=self.table_name).write()
		elif self.table_name == 'mgt_sch':
